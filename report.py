...
class reportClass:
    def __init__(self,root):
        ...
        btn_search=Button(self.root,text='Search',font=('goudy old style',15,"bold"),bg='#03e9f4',cursor='hand2',command=self.search).place(x=680,y=100,width=100,height=35)
        btn_clear=Button(self.root,text='Clear',font=('goudy old style',15,"bold"),bg='gray',cursor='hand2',command=self.clear).place(x=800,y=100,width=100,height=35)
        ...
        #=====button _delete=====
        btn_delete=Button(self.root,text='Delete',font=('goudy old style',15,"bold"),bg='red',cursor='hand2',command=self.delete).place(x=530,y=350,width=150,height=35)
    ...
    def search(self):
        ...
        try:
            if self.var_search.get()=="":
                ...
                cur.execute("select * from result1  where roll=?",(self.var_search.get(),))
                row=cur.fetchone()
                if row != None:
                    self.var_id=row[0]
    ...
    def delete(self):
        ...
        try:
            if self.var_id=="":
                ...
                cur.execute("select * from result1 where rid=?",(self.var_id,))
                row=cur.fetchone()
                if row==None:
                    ...
                    if op==True:
                        cur.execute("delete from result1 where rid=?",(self.var_id,))
                        con.commit()
