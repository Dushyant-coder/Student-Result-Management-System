...
class resultClass:
    def __init__(self,root):
        ...
        btn_search=Button(self.root,text='Search',font=('goudy old style',20,"bold"),bg='#03e9f4',cursor='hand2',command= self.search).place(x=500,y=100,width=100,height=28)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                cur.execute("select*from result1 where roll=? and course=?",(self.var_roll.get(),self.var_course.get()))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Result already present",parent=self.root)
                else:
                    per=(int(self.var_marks.get())*100)/int(self.var_full_marks.get())
                    cur.execute("insert into result1 (roll,name,course,marks_ob,full_marks,per) values(?,?,?,?,?,?)",(
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_course.get(),
                        self.var_marks.get(),
